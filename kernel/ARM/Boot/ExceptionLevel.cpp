...
extern "C"
{
    ...
    */
    void switch_to_el1()
    {
        auto const initialExceptionLevel = AArch64::ASM::GetCurrentExceptionLevel();

        if (initialExceptionLevel > AArch64::ASM::ExceptionLevel::EL3)
        {
            AArch64::Panic("Unknown exception level (above EL3)");
        }
        else if (initialExceptionLevel < AArch64::ASM::ExceptionLevel::EL1)
        {
            AArch64::Panic("We must at least be in EL1 to boot");
        }

        if (initialExceptionLevel > AArch64::ASM::ExceptionLevel::EL2)
        {
            // #TODO: Figure out how to handle EL3
            AArch64::Panic("We don't yet know how to switch from EL3 to EL2");
        }
        if (initialExceptionLevel > AArch64::ASM::ExceptionLevel::EL1)
        {
            AArch64::SwitchFromEL2ToEL1();
        }

        // Disable all traps so that EL1 and EL0 can access the coprocessor, floating point, and SIMD instructions and
        // registers
        AArch64::ASM::SetCPACR_EL1(CPACR_EL1_INIT_VALUE);
    }
}